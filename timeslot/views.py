...
def availabletime(request):
    try:
        ...
        start_time_candidate = datetime.strptime(str(users[0]['starttime']), "%H:%M:%S")
        end_time_candidate = datetime.strptime(str(users[0]['endtime']), "%H:%M:%S")
        
        start_time_interviewer = datetime.strptime(str(users[1]['starttime']), "%H:%M:%S")
        end_time_interviewer = datetime.strptime(str(users[1]['endtime']), "%H:%M:%S")

        for singletime in daterange(start_time_candidate, end_time_candidate):
            timeslots_candidate.append(singletime)

        for singletime in daterange(start_time_interviewer, end_time_interviewer):
            timeslots_interviewer.append(singletime)       

        return JsonResponse({"message":[item for item in timeslots_interviewer if item in timeslots_candidate]},status=200)
        pass
    except Exception as e:
        return JsonResponse({'message': 'Server Exception.'+str(e)}, status=500)
